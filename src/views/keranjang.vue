            ...
            <v-list-item-content>
              ...
              <v-list-item-subtitle>
                <span class="text-caption grey--text text--darken-1">
                  <span class="red--text">*</span> tidak bisa menambah produk sesudah memilih kurir
                </span>
              </v-list-item-subtitle>
            </v-list-item-content>
          </v-list-item>
...
export default {
  ...
  methods: {
    filterUserAlamat(){
      ...
      this.pilihKurir = "";
      this.pilihServis = "";
      let currentAlamat = this.userAlamat.filter((curr) => { return curr.alamatId == this.pilihAlamat});
      this.pilihProvinsi = currentAlamat[0].provinsi;
      this.pilihKota = currentAlamat[0].kabupaten;
      this.detailAlamat = currentAlamat[0].telp + ", " + currentAlamat[0].detailAlamat;
    },
    async getAlamat(){
      const database = await db.collection("alamat").where("userId", "==", firebase.auth().currentUser.uid).get();
